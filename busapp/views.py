...
class TransactionList(generics.ListCreateAPIView):
	...
	def pre_save(self, obj):
                obj.owner = User.objects.get(username=self.request.user).customer
...
def customer_app(request):
				...
				if stop.bus_stop.name == dest:
					for sched in Schedule.objects.filter():
						if date == unicode(sched.datetime.date()):
							newdata = {"year":sched.datetime.date().year,"month":sched.datetime.date().month,"day":sched.datetime.date().day,"source":src,"destination":dest,"amount":(sched.bus.rate*stop.distance),"remaining":sched.capacity,"time":sched.datetime.time(),"sid":sched.id,"company":sched.bus.owner,"discount":(sched.bus.rate*stop.distance*(100-sched.bus.discount)/100)}
	...
	elif 'customer' in request.POST and 'schedule' in request.POST and 'cost' in request.POST and 'seat' in request.POST and request.POST['customer'] and request.POST['cost'] and request.POST['seat']:
		cus = Customer.objects.get(user=request.POST['customer'])
		sch = Schedule.objects.get(id=request.POST['schedule'])
		tcost = request.POST['cost']
		tseat = request.POST['seat']
...
def companyTransaction(request,pk):
        if request.method == 'GET':
                companyId = pk
                companyObj = Company.objects.get(id=companyId)
                buses = companyObj.buses.iterator()
                scList= []
                final=[]
                ans=[]
                for k in buses:
                        for x in k.schedule_set.iterator():
                                scList.append(x)
                for k in scList:
                        final.append(k.transaction_set.iterator())
                for k in final:
                        ans.append(TransactionSerializer(k).data)
                return JSONResponse(ans)
